...
"""
Fixed Asset Register Extended Report

This is an extended version of the standard ERPNext Fixed Asset Register report
with additional columns for Custodian and Condition.

Additional Features:
- Custodian column (Employee link)
- Condition column (custom_condition field)
- Group by Custodian or Condition
"""
...
def get_data(filters):
	...
	# Extended fields list - includes custodian and custom_condition
	fields = [
		"name as asset_id",
		"asset_name",
		"status",
	...
	# Get custodian names map
	custodian_map = get_custodian_name_map()
	...
	for asset in assets_record:
		...
		row = {
			...
			"custodian_name": custodian_map.get(asset.custodian, ""),
			"condition": asset.custom_condition,
		}
		data.append(row)
...
def get_conditions(filters):
	...
	# 	conditions["custodian"] = filters.get("custodian")
	# if filters.get("condition"):
	# 	conditions["custom_condition"] = filters.get("condition")
...
def prepare_chart_data(data, filters):
	...
	for d in data:
		if d.get(date_field):
			...
			if belongs_to_month in labels_values_map:
...
def get_asset_depreciation_amount_map(filters, finance_book):
	...
	if filters.get("location"):
		query = query.where(asset.location == filters.location)
	# if filters.get("custodian"):
	# 	query = query.where(asset.custodian == filters.custodian)
	# if filters.get("condition"):
	# 	query = query.where(asset.custom_condition == filters.condition)
	if filters.status:
...
def get_group_by_data(
	...
	# Map group_by to actual field name if it's "condition"
	actual_field = "custom_condition" if group_by == "condition" else group_by
	...
	for a in assets:
		...
		# Use the actual field for grouping
		group_value = a.get(actual_field)
		idx = ([i for i, d in enumerate(data) if d.get(actual_field) == group_value] or [None])[0]
		if idx is None:
			data.append(a)
		else:
			for field in (
...
def get_custodian_name_map():
	"""Get mapping of Employee ID to Employee Name"""
...
def get_columns(filters):
	...
	# Handle group by for extended fields
	if group_by == "Custodian":
		return [
			{
				"label": _("Custodian"),
				"fieldtype": "Link",
	...
	if group_by in ["Asset Category", "Location"]:
		return [
			{
				"label": _("{}").format(group_by),
				"fieldtype": "Link",
	...
	# Default detailed view with extended columns
	return [
		{
			"label": _("Asset ID"),
			"fieldtype": "Link",
		...
		# Extended columns - Custodian and Condition
		{
			"label": _("Custodian"),
			"fieldtype": "Link",
			"fieldname": "custodian",
		...
		{"label": _("Custodian Name"), "fieldtype": "Data", "fieldname": "custodian_name", "width": 120},
		{"label": _("Condition"), "fieldtype": "Data", "fieldname": "condition", "width": 100},
		# End extended columns
