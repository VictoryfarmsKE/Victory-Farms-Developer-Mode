...
def execute(filters=None):
	...
	- only_include_bank (optional substring, defaults 'equity')
	- GET earning & deduction component columns
	"""
	if not filters:
		filters = {}
	...
	for r in rows:
		...
		if show_components:
			...
			for comp, amt in emp_deductions.items():
				r[frappe.scrub(comp)] = amt
		# Mandatory bonus components  show_components even if = 0
		for comp in mandatory_bonus_components:
			fieldname = frappe.scrub(comp)
...
def get_data(filters):
	...
	only_include_bank = filters.get("only_include_bank", "equity")

	params = {
		"from_date": filters.get("from_date"),
		"to_date": filters.get("to_date"),
		"salary_currency": salary_currency,
		"only_include_bank": f"%{only_include_bank}%",
	}
	...
	WHERE ss.docstatus = 1
		...
		AND emp.bank_name LIKE %(only_include_bank)s
