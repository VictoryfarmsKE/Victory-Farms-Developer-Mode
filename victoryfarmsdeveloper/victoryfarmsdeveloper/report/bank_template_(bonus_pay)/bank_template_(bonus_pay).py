...
def execute(filters=None):
    ...
    if data:
        ...
        for row in data:
            ...
            row["debit_account_no"] = "1410265957678"
            row["beneficiary_account_no"] = emp_info.get("bank_ac_no")
            row["beneficiary_name"] = row.get("employee_name") or emp_info.get("employee_name")
            bank_code = emp_info.get("custom_bank_code") or ""
            branch_code = emp_info.get("custom_branch_code") or ""
            row["banknbranch_code"] = f"{bank_code}{branch_code}" if bank_code or branch_code else None
            row["purpose_code"] = "OTH"
            ...
            row["bank_name"] = emp_info.get("bank_name")
...
def get_appraisal_payout_details(filtered_appraisal_payouts):
    ...
    return (
        ...
        .where(Employee.bank_name.not_like('%equity%'))
        .select(
...
def get_columns():
    cols = [
        ...
        {"fieldname": "bank_name", "label": _("Bank Name"), "fieldtype": "Data", "width": 150},
        {"fieldname": "beneficiary_name", "label": _("Beneficiary Name"), "fieldtype": "Data", "width": 200},
        {"fieldname": "banknbranch_code", "label": _("BanknBranch Code"), "fieldtype": "Data", "width": 80},
        {"fieldname": "purpose_code", "label": _("Purpose Code"), "fieldtype": "Data", "width": 100},
