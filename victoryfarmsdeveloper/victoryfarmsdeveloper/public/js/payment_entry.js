        ...
        on_success(file_doc) {
            ...
            frappe.call({
                ...
                callback(r) {
                    ...
                    set_beneficiary_purpose(frm);
                    update_paid_amount_from_beneficiaries(frm);
                    frm.refresh_field('custom_beneficiaries');
                    frappe.show_alert({
...
frappe.ui.form.on('Payment Entry Beneficiary', {
    ...
    custom_beneficiaries_add(frm, cdt, cdn) {
        update_paid_amount_from_beneficiaries(frm);
    },
    custom_beneficiaries_remove(frm, cdt, cdn) {
        update_paid_amount_from_beneficiaries(frm);
    }
});


function update_paid_amount_from_beneficiaries(frm) {
    if (frm.doc.custom_upload_type !== 'Mpesa') return;
