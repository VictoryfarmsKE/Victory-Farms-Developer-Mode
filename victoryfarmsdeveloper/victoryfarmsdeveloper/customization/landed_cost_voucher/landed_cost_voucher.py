...
class CustomLandedCostVoucher(Document):
	...
	if TYPE_CHECKING:
		...
		company: DF.Link
		distribute_charges_based_on: DF.Literal["Qty", "Amount", "Volume", "Distribute Manually"]
		...
		for pr in self.get("purchase_receipts"):
			if pr.receipt_document_type and pr.receipt_document:
				...
				for d in pr_items:
					...
					# Set custom_volume from Item master if available
	...
	def set_applicable_charges_on_item(self):
		if self.get("taxes") and self.distribute_charges_based_on != "Distribute Manually":
			...
			if total_item_cost == 0:
				frappe.throw(
					_(
						"Total {0} for all items is zero. Cannot distribute charges proportionately. Please ensure items have valid {0} values or choose a different distribution method."
			...
			for item in self.get("items"):
				...
				item.applicable_charges = flt(
					field_value * (flt(self.total_taxes_and_charges) / flt(total_item_cost)),
