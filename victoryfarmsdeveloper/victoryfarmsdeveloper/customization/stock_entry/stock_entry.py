...
def before_save_stock_entry(doc, method):
        ...
        if doc.stock_entry_type == "Material Transfer" and doc.items and all(item.item_group == "Gutted Fish-Tilapia" for item in doc.items) and doc.destination_warehouse_type in ["Branch", "LC"] and doc.from_warehouse_type in ["LC"]:
