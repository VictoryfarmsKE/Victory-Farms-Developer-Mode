...
class CustomStockEntry(StockEntry):
    ...
    def update_child_table(self):
        frappe.log_error("update_child_table triggered", "Custom Debug")
        self.crates = []
    ...
    def validate_crates(self):
        ...
        for item_code, qty in items.items():
            if item_code in crates and qty != crates[item_code]:
                frappe.throw(f"Quantity {qty} for {item_code} does not match quantity in crates!")
    ...
    def get_crates(self):
        ...
        if self.crates:
            for crate in self.crates:
                ...
                crates[item_code] = crates.get(item_code, 0) + crate.get("qty")
        return crates
