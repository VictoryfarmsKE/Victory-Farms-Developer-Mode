...
class CustomLeaveApplication(Document):
	...
	def show_insufficient_balance_message(self, leave_balance_for_consumption: float) -> None:
		...
		if frappe.db.get_value("Leave Type", self.leave_type, "allow_negative"):
			...
			self.status = "Pending Sufficient Balance"
			frappe.msgprint(
				_("Insufficient leave balance for Leave Type {0}. Status set to Pending Sufficient Balance.").format(frappe.bold(self.leave_type)),
				title=_("Insufficient Balance"),
				indicator="orange",
			)
			frappe.local.flags.silent = True
			# raise frappe.ValidationError("Insufficient leave balance, status set to Pending Sufficient Balance.")
