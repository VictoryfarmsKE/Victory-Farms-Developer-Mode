...
def validate_mandatory_fields(doc, method):
    ...
    _require(doc, "custom_job_title", _("Job Title"))
    _require(doc, "reports_to", _("Reports to"))
    _require(doc, "grade", _("Grade"))
    _require(doc, "custom_location", _("Location"))
    _require(doc, "custom_terms_of_service", _("Terms of Service"))
    ...
    _require(doc, "person_to_be_contacted", _("Emergency Contact Name"))
    ...
    _require(doc, "shif_no", _("SHIF No"))
    _require(doc, "tax_id", _("Tax ID"))
    ...
    if doc.residential_status == "Resident":
        if doc.meta.has_field("tax_id") and not doc.tax_id:
            frappe.throw(
                _("Tax ID is mandatory for employees with Residential Status = Resident."),
                title=_("Missing Mandatory Field")
            )
        if doc.meta.has_field("shif_no") and not doc.shif_no:
            frappe.throw(
                _("SHIF No is mandatory for employees with Residential Status = Resident."),
                title=_("Missing Mandatory Field")
            )
